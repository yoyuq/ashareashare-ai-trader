...
> 平台层代码架构速览 (v6.1)。策略研究方法论与运行记录为私有研究产物, 不在本仓库公开范围。
...
├─────────────────────────────────────────────────────────────────┤
│ 研究支撑层 (脚本化, 只描述能力不公开研究结论)                       │
│   scripts/ (历史数据 PIT 建库 · A/B 实验回测 · 前瞻 OOS 跟踪 ·      │
│             拥挤度/注意力监控 · 任务计划运维)                       │
├─────────────────────────────────────────────────────────────────┤
│ 数据层                                                          │
...
每组一个职责族 (实验判据/结果/操作单在私有 `reports/`, 不入库):

- **历史数据建库** (`build_*` / `fetch_*`): 东财/akshare 直连抓真·时点历史并
  冻结为 `replay_data/*.parquet` (龙虎榜 / 股东户数 / 披露时间表 / 解禁 /
  退市股日线 / 实时面板增量) — 回测只消费冻结库, 缺数据报错不兜底 (零模拟)。
- **A/B 实验回测** (`run_*` / `probe_*`): 每实验一个脚本, 判据在回测前写死
  并逐字执行; probe 脚本先验数据门 (可得性/覆盖度/时点表达力) 再决定建库。
- **前瞻 OOS 验证** (`forward_register_*` / `forward_track.py`): 回测通过 →
  预注册冻结 → 纸面平行 bet → 每交易日快照跟踪 → 判定日按预注册判据裁决。
- **监控测量层** (`crowding_watch.py` / `attention_collector.py`): 只读预警,
  不参与选券。
- **运维** (`fix_scheduled_tasks.ps1` 等): Windows 任务计划注册/修复。
